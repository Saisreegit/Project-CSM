...
@login_required(role='admin2')
def admin2_dashboard():
    # Admin2 can only see trainings they created
    admin2_username = get_current_username()
    my_trainings = {
        title: data for title, data in trainings.items() if data.get('created_by') == admin2_username
    }

    trainings_with_completion = {}
    for title, training_data in my_trainings.items():
    ...
    return render_template('admin2_dashboard.html', 
                           username=get_current_username(),
                           trainings=trainings_with_completion)

@admin2_bp.route('/create_training_folder', methods=['POST'])
@login_required(role='admin2')
def admin2_create_training_folder():
    ...
    try:
        ...
        trainings[training_title] = {
            ...
            'created_by': session.get('username'), # Admin2 creates it
            'videos': []
        }
...
@admin2_bp.route('/manage_training/<title>')
@login_required(role='admin2')
def admin2_manage_training(title):
    training_data = trainings.get(title)
    if not training_data:
        flash("Training not found.", 'error')
        return redirect(url_for('admin2.admin2_dashboard'))
    
    # Ensure admin2 only manages their own training
    if training_data.get('created_by') != get_current_username():
        flash("Access denied. You can only manage trainings you created.", 'error')
        return redirect(url_for('admin2.admin2_dashboard'))

    return render_template('admin2_manage_training.html', 
                           training_title=title, 
                           training_data=training_data,
                           uploads_folder=current_app.config['UPLOAD_FOLDER'])

@admin2_bp.route('/upload_video/<training_title>', methods=['POST'])
@login_required(role='admin2')
def admin2_upload_video(training_title):
    ...
    if training_data.get('created_by') != get_current_username():
        flash("Access denied. You can only upload videos to trainings you created.", 'error')
        return redirect(url_for('admin2.admin2_dashboard'))

    if 'video_file' not in request.files:
        flash('No video file part', 'error')
        return redirect(url_for('admin2.admin2_manage_training', title=training_title))
...
@admin2_bp.route('/delete_video/<training_title>/<video_filename>')
@login_required(role='admin2')
def admin2_delete_video(training_title, video_filename):
    training_data = trainings.get(training_title)
    if not training_data:
        flash("Training not found.", 'error')
        return redirect(url_for('admin2.admin2_dashboard'))
    ...
    if training_data.get('created_by') != get_current_username():
        flash("Access denied. You can only delete videos from trainings you created.", 'error')
        return redirect(url_for('admin2.admin2_dashboard'))
...
@admin2_bp.route('/delete_training/<title>')
@login_required(role='admin2')
def admin2_delete_training(title):
    training_data = trainings.get(title)
    if not training_data:
        flash("Training not found.", 'error')
        return redirect(url_for('admin2.admin2_dashboard'))
    ...
    if training_data.get('created_by') != get_current_username():
        flash("Access denied. You can only delete trainings you created.", 'error')
        return redirect(url_for('admin2.admin2_dashboard'))
    ...
    for username in list(progress.keys()):
        if title in progress[username]:
            del progress[username][title]
            if not progress[username]:
...
@admin2_bp.route('/manage_video_quizzes/<training_title>/<video_filename>', methods=['GET', 'POST'])
@login_required(role='admin2')
def admin2_manage_video_quizzes(training_title, video_filename):
    training_data = trainings.get(training_title)
    if not training_data:
        flash("Training not found.", 'error')
        return redirect(url_for('admin2.admin2_dashboard'))
    ...
    if training_data.get('created_by') != get_current_username():
        flash("Access denied. You can only manage quizzes for trainings you created.", 'error')
        return redirect(url_for('admin2.admin2_dashboard'))
    ...
    if request.method == 'POST':
        if 'add_quiz' in request.form:
            ...
            if not question:
                ...
                return redirect(url_for('admin2.admin2_manage_video_quizzes', training_title=training_title, video_filename=video_filename))
    ...
    return render_template('admin2_manage_quizzes.html',
                           training_title=training_title,
                           video_filename=video_filename,
                           video_info=video_info)

@admin2_bp.route('/delete_quiz/<training_title>/<video_filename>/<int:quiz_index>')
@login_required(role='admin2')
def admin2_delete_quiz(training_title, video_filename, quiz_index):
    training_data = trainings.get(training_title)
    if not training_data:
        flash("Training not found.", 'error')
        return redirect(url_for('admin2.admin2_dashboard'))
    ...
    if training_data.get('created_by') != get_current_username():
        flash("Access denied. You can only delete quizzes from trainings you created.", 'error')
        return redirect(url_for('admin2.admin2_dashboard'))
...
@admin2_bp.route('/uploads/<folder_name>/<filename>')
def uploaded_file_admin2(folder_name, filename): # Renamed to avoid collision if both admin1 and admin2 blueprints are served by the same app
    # This route serves static files from the UPLOAD_FOLDER
    # Added admin2-specific check if needed, but typically shared for uploads
